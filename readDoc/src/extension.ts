...
export function activate(context: vscode.ExtensionContext) {
    ...
    try {
        ...
        const docProvider = new DocumentationProvider(context);
        ...
        const showDocs = vscode.commands.registerCommand('codedocs.showDocs', async (dependency?: Dependency) => {
            try {
                const panel = await DocumentationPanel.createOrShow(context.extensionUri);
                if (dependency) {
        ...
        async function analyzeWorkspace(path: string) {
            try {
